...
class CWMachine(StateMachine):
    ...
    ## Event that moves between off state and a pause between messages
    doMP = (
        off.to(msgPause) | msgPause.to(off)
    )
    ...
    def on_enter_dot(self):
        ...
        # Blink for DOT_TIME once, don't return until time is elapsed
        self.redLight.blink(DOT_TIME, 0, 1, False)
    ...
    def on_enter_dash(self):
        ...
        # Blink for DASH_TIME once, don't return until time is elapsed
        self.blueLight.blink(DASH_TIME, 0, 1, False)
    ...
    # A state action for adding a pause between messages
    def on_enter_msgPause(self):
        # Wait PAUSE_MSG time
        sleep(PAUSE_MSG)
        
        if(DEBUG):
            print("* Pausing between messages")


    def on_exit_msgPause(self):
        pass
    ##
    ...
    def transmit(self):
        ...
        # Parentheses are added to keep track of scope, python is bad at this
        #(
            ...
            #(
                ...
                #(
                    ...
                    #(
                        ...
                        ## Dot or dash?
                        if (x == '.'):
                            #Enter dot state from 'off' state
                            self.doDot()
                            #Return to 'off' state
                            self.doDot()
                        elif (x == '-'):
                        ...
                        if (morseCounter < lenMorse):
                            #If not at end of letter, increment for loop counter and do dot dash pause
                            #Then return to off state
                    ...
                    if (wordCounter < lenWord):
                        #If not at end of word, increment for loop counter and do letter pause
                        #Then return to off state
                ...
                if (wordsCounter < lenWords):
                        #If not at end of message, increment for loop counter and do word pause
                        #Then return to off state
...
# Cleanup to be used to exit gracefully
def cleanup_machine():
    global repeat
    global cwMachine
