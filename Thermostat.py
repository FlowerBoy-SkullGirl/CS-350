...
class TemperatureMachine(StateMachine):
    ...
    def updateLights(self):
        ...
        # If state is cool, pulse when above temp target, solid on when at or below
        if self.current_state.id == "cool":
            if temp <= self.setPoint:
                blueLight.on()
            else:
                blueLight.pulse()
        # If state is heat, pulse when below temp target, solid on when at or above
        elif self.current_state.id == "heat":
            if temp < self.setPoint:
                redLight.pulse()
            else:
                redLight.on()
        # Only other condition should be off, so turn LED's off for all else conditions
        else:
            blueLight.off()
            redLight.off()
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                ...
                lcd_line_2 = f"S:{self.current_state.id} T:{self.setPoint}F"
